...
my $INSTANCE_LIFETIME = 345600; # 4 days
...
my $TRIGGER_REFRESH = "add|refresh|flag|reflag|update";
...
sub MESSAGE_INFO {
    my $slLittleDeath = quest::saylink("little death");
    my $slCorpse = quest::saylink("corpse");
    $client->Message(15, "$npcname says, 'Hello, mortal one.  Do you wish to test yourself against the [$slLittleDeath]?  If you have lost a [$slCorpse], I can try and retrieve it after the test is complete.'");
}
